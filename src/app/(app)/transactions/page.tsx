...
export default function TransactionsPage() {
  ...
  const { t } = useI18n();
  const { items, total, loading, query, setQuery } = useApiList<Tx>("/api/transactions");
      ...
      <CardHeader className="gap-4">
        ...
        <div className="grid gap-3 md:grid-cols-6">
          ...
          <Select
            ...
            onValueChange={(value) => setQuery({ ...query, type: value, page: 1 })}
          >
      ...
      <CardContent>
              ...
              <tbody>
                {items.map((item) => (
                  ...
                  >
                    ...
                    <td className="pr-4 font-medium max-w-[200px] truncate" title={item.description}>
                      {item.description}
                    </td>
                    <td className={cn(
                      "pr-4 font-bold tabular-nums",
                      item.type === "Income" ? "text-teal-600" : "text-orange-600"
                    )}>
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            {total > 0
              ? `${t("page")} ${query.page} ${t("of")} ${totalPages} · ${total} ${t("transactions").toLowerCase()}`
              : `0 ${t("transactions").toLowerCase()}`}
          </p>
