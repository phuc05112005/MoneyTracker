...
export function ForgotPasswordForm() {
  ...
  async function submit(values: z.infer<typeof forgotPasswordSchema>) {
    ...
    if (!response.ok) {
      toast({ title: "Reset email failed", description: data.error ?? "Please try again later." });
      return;
    }
    toast({ title: "Check your email", description: data.message ?? "If the email exists, a reset link will be sent." });
  }
    ...
    <Card className="w-full max-w-md border-white/40 bg-card/85 shadow-soft backdrop-blur-xl">
      ...
      <CardContent>
        <form className="space-y-4" onSubmit={form.handleSubmit(submit)}>
          ...
          <Button className="w-full" type="submit"><Mail className="h-4 w-4" />Send reset email</Button>
